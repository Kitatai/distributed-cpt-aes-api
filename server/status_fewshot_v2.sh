#!/bin/bash
# Show few-shot v2 experiment status
# Usage: ./status_fewshot_v2.sh [--server URL]
...
echo "=== Few-shot v2 Status ==="
echo ""

# Get task counts
curl -s "$SERVER/fewshot_v2/tasks" | python3 -c "
import sys, json
...
curl -s "$SERVER/fewshot_v2/summary" | python3 -c "
import sys, json
data = json.load(sys.stdin)
...
for model in models:
    ...
    for k in k_values:
        if model in by_k_model.get(k, {}):
            ...
            best = m.get('best_spearman', 0)
            best_std = m.get('best_spearman_std', 0)
            print(f'  {e0:.3f}±{e0_std:.3f}       {best:.3f}±{best_std:.3f}  ', end='')
        else:
            print(f'       -             -        ', end='')
    print()
