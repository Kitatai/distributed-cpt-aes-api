...
@app.post("/tasks/init")
async def init_tasks(force: bool = Query(False, description="Force reinitialize all tasks")):
    """Initialize all experiment tasks (24 tasks: 8 prompts x 3 models)."""
    lock = FileLock(str(TASKS_DIR / ".init.lock"))
    ...
    with lock:
        ...
        if existing and not force:
            return {
                "message": f"Tasks already exist ({len(existing)} tasks). Use force=true to reinitialize.",
                "initialized": False,
            }

        # Clear existing tasks if force
        if force:
            for f in existing:
                f.unlink()
        ...
        logger.info(f"Initialized {created} tasks")
        return {
            "message": f"Initialized {created} tasks",
            "initialized": True,
            "task_count": created,
        }
...
async def get_next_task(worker_id: str = Query(..., description="Worker identifier")):
